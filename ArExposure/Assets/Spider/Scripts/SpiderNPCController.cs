...
public class SpiderNPCController : MonoBehaviour {
    ...
    */
    ...
    private void FixedUpdate()
    {
        ...
        // Use spider's position for raycasting
        Vector3 raycastOrigin = spider.transform.position;

        // Raycast down to find the ground
        if (Physics.Raycast(raycastOrigin, -Y, out RaycastHit hit, Mathf.Infinity))
        {
            // Use hit.normal to align the spider with the surface normal
            Vector3 surfaceNormal = hit.normal;

            // Get random movement direction
            Vector3 input = getRandomDirection() * getRandomBinaryValue(0, 1, 0.4f);

            // Project input onto the surface normal to make the spider walk on surfaces
            input = Vector3.ProjectOnPlane(input, surfaceNormal).normalized;

            spider.walk(input);
            spider.turn(input);

            if (showDebug) Debug.DrawLine(raycastOrigin, raycastOrigin + input * 0.1f * spider.getScale(), Color.cyan, Time.fixedDeltaTime);
        }
    }
    private void Update() {
        if (showDebug) {
