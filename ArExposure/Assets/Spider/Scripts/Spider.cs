...
public class Spider : MonoBehaviour {
    ...
    [Header("ARexposure")]
    public GameObject surface; // Object to generate movement boundary
    ...
    private void Awake() {
        ...
        // Example values, replace them with your actual boundary parameters
        //Vector3(-0.00300000003,1.51400006,0.592999995)
        GameObject surf = GameObject.Find("Surface"); // Replace "CubeObjectName" with the actual name of your cube object
        if (surf != null)
        {
            // Set boundaryCenter to the center of the cube
            boundaryCenter = surf.transform.position;
            Debug.Log("Boundary center is: " + surf.transform.position);
        }
        else
        {
            Debug.LogError("Surf object not found in the scene. ");
    ...
    private void move(Vector3 direction, float speed)
    {
        // TODO: Make sure direction is on the XZ plane of the spider! For this maybe refactor the logic from input from spidercontroller to this function.
        ...
        if (direction != Vector3.zero)
        {
            ...
            if (Vector3.Distance(newPosition, boundaryCenter) <= boundaryRadius)
            {
                // If within the boundary, move to the new position
                transform.position = newPosition;
            }
            else
            {
                // If outside the boundary, project the velocity onto the boundary plane
                Vector3 projectedVelocity = Vector3.ProjectOnPlane(currentVelocity, Vector3.up);
                currentVelocity = projectedVelocity.normalized * currentVelocity.magnitude;

                // Move the spider based on the adjusted velocity
